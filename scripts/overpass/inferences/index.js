...
export function performInferences(tags) {
    ...
    for (const infer of inferences)
        infer.setDefault(tags, inferredTags);
    return inferredTags;
}
/**
...
const allInferences = {
    ...
    ], [], new OsmBoolean(false), (noTransform), (oneway, { lanesBackward }, warnings) => {
     ...
     * @param inferredTags Set of tags which have been inferred to add this tag to if changes are made.
     */
    inferJunction: new InferenceCollection("junction", [], [], new OsmString("no"), (noTransform), noValidation),
    /**
     ...
     * @param inferredTags Set of tags which have been inferred to add this tag to if changes are made.
     */
    inferSurface: new InferenceCollection("surface", [], [], new OsmString("unknown"), (noTransform), noValidation),
    /**
    ...
    ], new OsmUnsignedInteger(2), (noTransform), (lanes, { lanesForward, lanesBackward }, warnings) => {
        // lanes === 0
        if (lanes.eq(0))
            warnings.add(TagWarning.lanesEqualZero("lanes"));
        // lanes !== lanes:forward + lanes:backward
        if (!lanes.eq(lanesForward.add(lanesBackward)))
            warnings.add(TagWarning.lanesUnequalToForwardBackward(lanes, lanesForward, lanesBackward));
    }),
    /**
    ...
    ], new OsmUnsignedInteger(1), (noTransform), (lanesForward, _tags, warnings) => {
        // lanes:forward === 0
    ...
    inferTurnLanesForward: new InferenceCollection("turnLanesForward", [
        ...
        InferenceBuilder.new("turnLanesForward")
            .assertIsSet("lanesForward")
            .setInferenceFn(tags => OsmDoubleArray.ofLength(tags.lanesForward.get(), "", OsmString))
    ...
    inferTurnLanesBackward: new InferenceCollection("turnLanesBackward", [
        ...
        InferenceBuilder.new("turnLanesBackward")
            .assertIsSet("lanesBackward")
            .setInferenceFn(tags => OsmDoubleArray.ofLength(tags.lanesBackward.get(), "", OsmString))
...
const ALL_CALCULATIONS = Object.values(allInferences)
    .map(collection => collection.calculations)
    .flat();
