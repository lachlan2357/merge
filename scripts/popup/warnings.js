...
/** Popup definition for the warnings screen. */ class WarningsPopup extends Popup {
    title = "Warnings";
    build() {
        const emptyText = new ElementBuilder("p").text("There are no warnings for the data on the map.").build();
        // ensure data exists
        ...
        for (const way of dataCache.values()){
            ...
            const button = new ElementBuilder("button").tooltip("Inspect Way", "left").event("click", ()=>{
                POPUP_DIALOG.close();
                Sidebar.show(id);
            }).children(icon).build();
            // main
            ...
            for (const [tag, warnings] of way.warnings){
                for (const warning of warnings){
                    const listItem = new ElementBuilder("li").text(`${tag}: ${warning.message}`).build();
                    list.children(listItem);
                }
            }
            // append children
            const container = createCustomElement(PopupContainer).appendToHeader(heading, button).appendToMain(list.build());
...
/** Instance of {@link WarningsPopup}. */ export const WARNINGS_POPUP = new WarningsPopup();
