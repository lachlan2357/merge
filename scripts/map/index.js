...
import "./buttons.js";
/** Container to control drawing the map to a {@link HTMLCanvasElement}. */ class Canvas {
    /** Reference to the HTML Canvas element. */ element;
    /** Reference to the container for the {@link element}. */ container;
    /**
	 ...
	 */ constructor(id, containerId){
        ...
        this.element.addEventListener("mousemove", (e)=>{
            ...
            if (State.mouseDown.get()) State.mouseOffset.set(pos.subtract(State.mouseDownPos.get()));
    ...
    /** Re-centre the canvas back to origin, resetting offset and zoom. */ centre() {
	 ...
	 */ zoom(inOut, source) {
        ...
        const zoomPosition = source == "mouse" ? State.mousePos.get().subtract(State.canvasOffset.get()) : State.canvasDimensions.get().divide(2);
        const zoomCoord = zoomPosition.toWorld();
        if (inOut === "in") State.zoom.setDynamic((old)=>old + zoomIncrement);
        else if (totalMultiplierRaw - zoomIncrement > 0) State.zoom.setDynamic((old)=>old - zoomIncrement);
        else return;
        const newCoord = zoomCoord.toScreen();
        const diff = zoomPosition.subtract(newCoord);
        State.zoomOffset.setDynamic((old)=>old.add(diff));
    }
    /** Toggle whether the map should be displayed in fullscreen. */ toggleFullscreen() {
        this.container.toggleAttribute("fullscreen");
    }
    /** Draw the map onto the canvas, taking consideration of zoom, offset, etc. */ draw() {
        ...
        dataCache.forEach((way, wayId)=>{
            ...
            for(let i = 0; i < way.orderedNodes.length; i++){
                ...
                const allOffScreen = new Matrix(2, 6, "unknown");
                for(let i = 0; i < 6; i++){
                ...
                const isOffscreen = !allOffScreen.getArray(0)?.includes("in") || !allOffScreen.getArray(1)?.includes("in");
                ...
                for(let i = 0; i < lanes; i++){
                    ...
                    if (leftTraffic) {
                        markings = i < lanesForward ? turnLanesForward[i] || new Array() : turnLanesBackward[turnLanesBackward.length + (lanesForward - i) - 1] || new Array();
                    } else {
                        markings = i < lanesBackward ? turnLanesBackward[i] || new Array() : turnLanesForward[turnLanesForward.length + (lanesBackward - i) - 1] || new Array();
                    }
                    const allX = [
                        thisSrtCoord.x,
                        thisEndCoord.x,
                    ...
                    const length = Math.sqrt((thisSrtCoord.x - thisEndCoord.x) ** 2 + (thisSrtCoord.y - thisEndCoord.y) ** 2) * (i < lanesForward ? directionality : -directionality);
                    const width = Math.sqrt((thisSrtCoord.x - nextSrtCoord.x) ** 2 + (thisSrtCoord.y - nextSrtCoord.y) ** 2) * (i < lanesForward ? directionality : -directionality);
                ...
                if (way.tags.oneway.eq(false)) drawLine(centreStartCoord, centreEndCoord, {
    ...
    /** Resize the canvas to fit it's container. */ setResolution() {
...
/** Main instance of the {@link HTMLCanvasElement}. */ export const CANVAS = new Canvas("canvas", "main");
// canvas effects
 ...
 */ class Matrix {
    /** The inner array storing the values of the matrix. */ inner;
    /**
	 ...
	 */ constructor(xLength, yLength, defaultValue){
        this.inner = [];
        for(let i = 0; i < xLength; i++){
            const arr = new Array(yLength).fill(defaultValue);
            this.inner.push(arr);
        }
    }
    /**
